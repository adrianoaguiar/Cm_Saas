 ...
 * @package    Mage_Admin
 */
class Cm_Saas_Model_User_Config extends Varien_Simplexml_Config
{
    /**
     ...
     * @return Cm_Saas_Model_User_Config
     */
    public function loadAclResources(Cm_Saas_Model_User_Acl $acl, $resource = null, $parentName = null)
    {
        if (is_null($resource)) {
            ...
            $acl->add(Mage::getModel('saas/user_acl_resource', $resourceName), $parentName);
        }

        if (isset($resource->all)) {
            $acl->add(Mage::getModel('saas/user_acl_resource', 'all'), null);
        }
    ...
    {
        $asserts = $this->getNode("saas/acl/asserts");
        if ('' === $name) {
            return $asserts;
        }
    ...
    {
        $sets = $this->getNode("saas/acl/privilegeSets");
        if ('' === $name) {
            return $sets;
        }
