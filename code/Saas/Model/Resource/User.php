 ...
 * @package     Cm_Saas
 */
class Cm_Saas_Model_Resource_User extends Mage_Core_Model_Resource_Db_Abstract
{
    /**
     * Define main table
     *
    ...
    {
        $this->_init('saas/user', 'user_id');
    }

    /**
     * Initialize unique fields
     *
     * @return Cm_Saas_Model_Resource_User
     */
     ...
     * @return Cm_Saas_Model_Resource_User
     */
    public function recordLogin(Cm_Saas_Model_User $user)
    {
    ...
    public function hasAssigned2Role($user)
    {
        ...
        if ( $userId > 0 ) {
            ...
            $select = $adapter->select();
            $select->from($this->getTable('saas/user_role'))
    ...
    {
        return Mage::helper('core')->getHash($pwStr, Cm_Saas_Model_User::HASH_SALT_LENGTH);
    }
    ...
    public function delete(Mage_Core_Model_Abstract $user)
    {
        ...
        try {
            ...
            $adapter->delete($this->getTable('saas/user_role'), $conditions);
    ...
    public function _saveRelations(Mage_Core_Model_Abstract $user)
    {
        ...
        try {
            ...
            foreach ($rolesIds as $rid) {
                $rid = intval($rid);
                if ($rid > 0) {
                    $row = Mage::getModel('saas/user_role')->load($rid)->getData();
                } else {
                ...
                $insertData = $this->_prepareDataForTable($data, $this->getTable('saas/user_role'));
                $adapter->insert($this->getTable('saas/user_role'), $insertData);
            }
            $adapter->commit();
        } catch (Mage_Core_Exception $e) {
            throw $e;
    ...
    public function getRoles(Mage_Core_Model_Abstract $user)
    {
        ...
        $table  = $this->getTable('saas/user_role');
        $adapter   = $this->_getReadAdapter();
    ...
    public function add(Mage_Core_Model_Abstract $user)
    {
        ...
        if ( sizeof($aRoles) > 0 ) {
            foreach ($aRoles as $idx => $data) {
                ...
                $dbh->delete($this->getTable('saas/user_role'), $conditions);
            }
        }

        if ($user->getId() > 0) {
            $role = Mage::getModel('saas/user_role')->load($user->getRoleId());
        } else {
        ...
        $dbh->insert($this->getTable('saas/user_role'), $insertData);
    ...
    public function deleteFromRole(Mage_Core_Model_Abstract $user)
    {
        ...
        $dbh->delete($this->getTable('saas/user_role'), $condition);
        return $this;
    }
