...
/**
 * Index saas controller
 *
 * @category    Cm
 * @package     Cm_Saas
 */
class Cm_Saas_Saas_IndexController extends Cm_Saas_Controller_Action
{
    /**
    ...
    protected function _outTemplate($tplName, $data = array())
    {
        ...
        $block = $this->getLayout()->createBlock('saas/template')->setTemplate("$tplName.phtml");
    ...
    {
        $session = Mage::getSingleton('saas/session');
    ...
    public function logoutAction()
    {
        ...
        $adminSession = Mage::getSingleton('saas/session');
        $adminSession->unsetAll();
        $adminSession->getCookie()->delete($adminSession->getSessionName());
        $adminSession->addSuccess(Mage::helper('saas')->__('You have logged out.'));
    ...
    public function globalSearchAction()
    {
        ...
        if (!Mage::getSingleton('saas/session')->isAllowed('admin/global_search')) {
            $items[] = array(
                'id' => 'error',
                'type' => Mage::helper('saas')->__('Error'),
                'name' => Mage::helper('saas')->__('Access Denied'),
                'description' => Mage::helper('saas')->__('You have not enough permissions to use this functionality.')
            );
            $totalCount = 1;
        } else {
            if (empty($searchModules)) {
                $items[] = array(
                    'id' => 'error',
                    'type' => Mage::helper('saas')->__('Error'),
                    'name' => Mage::helper('saas')->__('No search modules were registered'),
                    'description' => Mage::helper('saas')->__('Please make sure that all global admin search modules are installed and activated.')
                );
                $totalCount = 1;
            } else {
                ...
                foreach ($searchModules->children() as $searchConfig) {
                    ...
                    if ($searchConfig->acl && !Mage::getSingleton('saas/session')->isAllowed($searchConfig->acl)){
                        continue;
                    }
        ...
        $block = $this->getLayout()->createBlock('saas/template')
    ...
    public function changeLocaleAction()
    {
        ...
        if ($locale) {
            Mage::getSingleton('saas/session')->setLocale($locale);
        }
    ...
    public function forgotpasswordAction()
    {
        ...
        if (!empty($email) && !empty($params)) {
            ...
            if (Zend_Validate::is($email, 'EmailAddress')) {
                ...
                $this->_getSession()
                    ->addSuccess(Mage::helper('saas')->__('If there is an account associated with %s you will receive an email with a link to reset your password.', Mage::helper('saas')->escapeHtml($email)));
            ...
            $this->_getSession()->addError(Mage::helper('saas')->__('The email address is empty.'));
        }
        $this->loadLayout();
        $this->renderLayout();
    }
    ...
    public function resetPasswordAction()
    {
        ...
        try {
            ...
            $this->_getSession()->addError(Mage::helper('saas')->__('Your password reset link has expired.'));
    ...
    public function resetPasswordPostAction()
    {
        ...
        if (iconv_strlen($password) <= 0) {
            array_push($errorMessages, Mage::helper('saas')->__('New password field cannot be empty.'));
        }
        ...
        try {
            ...
            $user->save();
            $this->_getSession()->addSuccess(Mage::helper('saas')->__('Your password has been updated.'));
            $this->_redirect('*/*/login');
        } catch (Exception $exception) {
    ...
    protected function _validateResetPasswordLinkToken($userId, $resetPasswordLinkToken)
    {
        if (!is_int($userId)
            ...
            throw Mage::exception('Mage_Core', Mage::helper('saas')->__('Invalid password reset token.'));
        }

        /** @var $user Mage_Admin_Model_User */
        $user = Mage::getModel('admin/user')->load($userId);
        if (!$user || !$user->getId()) {
            throw Mage::exception('Mage_Core', Mage::helper('saas')->__('Wrong account specified.'));
        }

        $userToken = $user->getRpToken();
        if (strcmp($userToken, $resetPasswordLinkToken) != 0 || $user->isResetPasswordLinkTokenExpired()) {
            throw Mage::exception('Mage_Core', Mage::helper('saas')->__('Your password reset link has expired.'));
        }
    }
